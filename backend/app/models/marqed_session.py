"""
MarQed Session Database Models

Week 144 - Fase 28: MarQed Persistentie
Week 158 - Renamed from BMAD to MarQed

Persistent storage for MarQed 8-question workflow sessions.
...
- MarQedSession: Question workflow (top-down, 8 strategic questions)
"""
...
class MarQedSessionStatus:
    """MarQed session status constants."""
...
class MarQedWorkflowType:
    """MarQed workflow type constants."""
    BROWN_PAPER = "brown_paper"   # Migration projects (8 questions)
    GREEN_PAPER = "green_paper"   # New projects (6 questions)


class MarQedSession(Base):
    """
    MarQed 8-question workflow session.
    ...
    # Workflow tracking
    status = Column(String(50), nullable=False, default=MarQedSessionStatus.QUESTIONS, index=True)
    current_question = Column(Integer, nullable=False, default=1)
    workflow_type = Column(String(50), nullable=False, default=MarQedWorkflowType.BROWN_PAPER, index=True)
    ...
    # Relationships
    answer_history = relationship("MarQedAnswer", back_populates="session", cascade="all, delete-orphan")
    events = relationship("MarQedSessionEvent", back_populates="session", cascade="all, delete-orphan")
    customer = relationship("Customer", back_populates="marqed_sessions")

    @property
    def total_questions(self) -> int:
        """Return total questions based on workflow type."""
        return 8 if self.workflow_type == MarQedWorkflowType.BROWN_PAPER else 6
    ...
    def can_resume(self) -> bool:
        ...
        return self.status in [MarQedSessionStatus.QUESTIONS, MarQedSessionStatus.REVIEW]
    ...
    def __repr__(self):
        return f"<MarQedSession(id={self.id}, project={self.project_name}, status='{self.status}', q={self.current_question}/{self.total_questions})>"


class MarQedAnswer(Base):
    """
    Individual MarQed answer with version history.

    Supports answer revisions - when a user updates an answer,
    the old version is kept with is_current=False.
    """
    __tablename__ = "marqed_answers"

    id = Column(Integer, primary_key=True, autoincrement=True)
    session_id = Column(String(36), ForeignKey('marqed_sessions.id', ondelete='CASCADE'), nullable=False, index=True)
    ...
    # Relationship
    session = relationship("MarQedSession", back_populates="answer_history")
    ...
    def __repr__(self):
        return f"<MarQedAnswer(session={self.session_id}, q={self.question_number}, v={self.version})>"


class MarQedSessionEvent(Base):
    """
    Audit trail for MarQed session events.

    Tracks all significant events in a session's lifecycle.
    """
    __tablename__ = "marqed_session_events"
    ...
    # Relationship
    session = relationship("MarQedSession", back_populates="events")
    ...
    def __repr__(self):
        return f"<MarQedSessionEvent(session={self.session_id}, type='{self.event_type}')>"


class MarQedEventType:
