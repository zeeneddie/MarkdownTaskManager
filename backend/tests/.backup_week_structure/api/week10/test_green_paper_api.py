...
async def test_project(db_session):
    """Create a test project (epic) item for Green Paper tests"""
    from app.crud import item as item_crud
    from app.models.item import ItemType

    project_id = f"EPIC-TEST-{uuid4().hex[:8]}"
    epic_data = {
        "id": project_id,
        "type": ItemType.EPIC,
        "title": f"Test Project {project_id}",
        "status": "PLANNED",
        "description": "Test project for Green Paper session"
    }
...
@pytest.mark.asyncio
async def test_start_session_success(client: AsyncClient, test_project):
...
@pytest.mark.asyncio
async def test_get_session_success(client: AsyncClient, test_project):
...
@pytest.mark.asyncio
async def test_submit_answer_success(client: AsyncClient, test_project):
