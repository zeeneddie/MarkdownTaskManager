...
''')
    ...
    def test_detect_python_unused_variable(self, service, temp_project):
        ...
        # Variable detection is limited - not all analyzers catch module-level vars
    ...
    def test_private_functions_are_safe_to_remove(self, service, temp_project):
        """Test that private functions are assessed for removal risk."""
        result = service.detect(project_path=temp_project)

        # Private functions should be detected and have a removal risk assessment
        for item in result.items:
            if item.name.startswith('_') and not item.name.startswith('__'):
                # Service assigns risk based on analysis - verify it has a risk
                assert item.removal_risk is not None

    def test_dunder_methods_are_not_safe(self, service):
        """Test that dunder methods are handled appropriately."""
        with tempfile.TemporaryDirectory() as tmpdir:
...
''')
            ...
            # If dunder methods are detected, verify they're flagged for review
            # (not safe to remove automatically)
            dunder_items = [
                item for item in result.items
                if item.name.startswith('__') and item.name.endswith('__')
            ]
            for item in dunder_items:
                assert item.removal_risk != RemovalRisk.SAFE
