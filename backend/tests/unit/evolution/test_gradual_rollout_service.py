...
@pytest.fixture
def sample_stage():
    """Create sample rollout stage with healthy metrics (error rate < 5%)."""
    return RolloutStage(
        id=uuid4(),
        rollout_id=uuid4(),
        stage_number=0,
        ...
        total_requests=150,
        successful_requests=145,  # 96.7% success
        failed_requests=5  # 3.3% error rate (under 5% threshold)
    )
...
@pytest.mark.asyncio
async def test_advance_to_next_stage_healthy(rollout_service):
    """Test advancing to next stage when current stage is healthy."""
    rollout_id = uuid4()

    # Create fully mocked rollout (can't use SQLAlchemy model with Mock stages)
    mock_rollout = Mock()
    mock_rollout.id = rollout_id
    mock_rollout.status = RolloutStatus.ACTIVE.value
    mock_rollout.current_stage = 0
    mock_rollout.stages = [Mock(), Mock()]  # 2 stages so we can advance

    # Create stage mock with non-completed status
    current_stage = Mock()
    current_stage.status = StageStatus.ACTIVE.value
    current_stage.started_at = datetime.now(timezone.utc) - timedelta(hours=1)

    # Mock database queries - first call returns rollout, second returns stage
    call_count = [0]
    def mock_execute_side_effect(*args, **kwargs):
        result = Mock()
        call_count[0] += 1
        if call_count[0] == 1:
            # First call: get rollout
            result.scalar_one_or_none = Mock(return_value=mock_rollout)
        else:
            # Second call: get current stage
            result.scalar_one = Mock(return_value=current_stage)
        return result

    rollout_service.db.execute = AsyncMock(side_effect=mock_execute_side_effect)
    rollout_service.db.commit = AsyncMock()
    ...
    with patch.object(rollout_service, 'check_rollout_health', new=AsyncMock(return_value=health)), \
         patch.object(rollout_service, '_start_stage', new=AsyncMock()) as mock_start:

        next_stage = await rollout_service.advance_to_next_stage(rollout_id)
...
async def test_trigger_rollback_performance_drop(rollout_service, sample_rollout, sample_stage):
    ...
    assert rollout.status == RolloutStatus.ROLLED_BACK.value
    assert "performance drop" in rollout.rollback_reason.lower()
...
async def test_get_rollout_status(rollout_service, sample_rollout):
    ...
    # Create a mock rollout with stages as a list attribute (not SQLAlchemy relationship)
    mock_rollout = Mock()
    mock_rollout.id = sample_rollout.id
    mock_rollout.experiment_id = sample_rollout.experiment_id
    mock_rollout.variant_id = sample_rollout.variant_id
    mock_rollout.agent_id = sample_rollout.agent_id
    mock_rollout.feature_name = sample_rollout.feature_name
    mock_rollout.current_stage = sample_rollout.current_stage
    mock_rollout.current_percentage = sample_rollout.current_percentage
    mock_rollout.status = sample_rollout.status
    mock_rollout.started_at = sample_rollout.started_at
    mock_rollout.completed_at = None  # Not completed yet
    mock_rollout.rolled_back_at = None
    mock_rollout.rollback_reason = None
    mock_rollout.metadata = sample_rollout.metadata
    mock_rollout.stages = [
    ...
    mock_result = Mock()
    mock_result.scalar_one_or_none = Mock(return_value=mock_rollout)
    rollout_service.db.execute = AsyncMock(return_value=mock_result)
...
async def test_get_rollout_status_completed(rollout_service):
    ...
    mock_rollout = Mock()
    mock_rollout.id = uuid4()
    mock_rollout.experiment_id = uuid4()
    mock_rollout.variant_id = uuid4()
    mock_rollout.agent_id = "felix"
    mock_rollout.feature_name = "test"
    mock_rollout.current_stage = 3
    mock_rollout.current_percentage = 100.0
    mock_rollout.status = RolloutStatus.COMPLETED.value
    mock_rollout.started_at = datetime.now(timezone.utc) - timedelta(hours=4)
    mock_rollout.completed_at = datetime.now(timezone.utc)
    mock_rollout.rolled_back_at = None
    mock_rollout.rollback_reason = None
    mock_rollout.metadata = {}
    mock_rollout.stages = [
        Mock(status=StageStatus.COMPLETED.value),
        Mock(status=StageStatus.COMPLETED.value),
        Mock(status=StageStatus.COMPLETED.value),
    ...
    status = await rollout_service.get_rollout_status(mock_rollout.id)
