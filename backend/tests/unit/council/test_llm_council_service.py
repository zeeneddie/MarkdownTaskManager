...
class TestStage1Response:
    ...
    async def test_query_single_model_timeout(self, council_service, mock_db_session):
        ...
        # Setup - use aiohttp mock properly with AsyncMock
        with patch('app.services.llm_council_service.aiohttp.ClientSession') as mock_client:
            ...
            mock_client.return_value.__aenter__ = AsyncMock(return_value=mock_session_instance)
...
class TestStage2PeerReview:
    ...
    async def test_peer_review_anonymization(self, council_service, mock_db_session):
        ...
        # Create mock responses from 3 actual models from council_service.models
        model_names = list(council_service.models.keys())[:3]  # Use first 3 real models
        mock_responses = []
    ...
    async def test_review_all_models(self, council_service, mock_db_session):
        ...
        # Use 4 actual models from council_service.models
        model_names = list(council_service.models.keys())[:4]
        mock_responses = [
            MagicMock(spec=CouncilResponse, id=uuid4(), model_name=model_name)
            for model_name in model_names
        ]
        mock_session.responses = mock_responses
...
class TestStage3Synthesis:
    ...
    async def test_outlier_detection(self, council_service):
        ...
        # Setup: Create conditions where outlier is clearly >2 std devs from mean
        # 5 responses with score 10.0, 1 with score 0.0
        # Mean = (5*10 + 0)/6 = 8.33
        # Variance = (5*(10-8.33)² + (0-8.33)²)/6 = (5*2.78 + 69.39)/6 = 13.84
        # StdDev = 3.72, 2*std = 7.44
        # abs(0 - 8.33) = 8.33 > 7.44 → detected as outlier!
        responses = []
        response_scores = {}

        # 5 similar responses with high scores
        for i in range(5):
            r = MagicMock(spec=CouncilResponse)
            r.id = uuid4()
            r.response_text = "RECOMMENDATION: Use microservices\nREASONING: Better scaling"
            responses.append(r)
            response_scores[str(r.id)] = 10.0

        # 1 outlier response with score 0
        ...
        response_scores[str(outlier.id)] = 0.0
        ...
        assert "monolithic" in outliers[0].lower()
    ...
    async def test_chairman_synthesis(self, council_service, mock_db_session):
        ...
        mock_responses = [
            MagicMock(spec=CouncilResponse, id=uuid4(), model_name="deepseek-r1:latest", confidence=0.85)
        ]
        mock_reviews = [
        ...
        # Mock Ollama chairman response using correct path
        with patch('app.services.llm_council_service.aiohttp.ClientSession') as mock_client:
            # Create proper async context managers for nested async with statements
